...
$(document).ready(function () {
    ...
    $('<tr><td width="20%">First Name</td><td>' + rFirstName + '</td></tr>' + '<tr><td>Last Name</td><td>' + rLastName + '</td></tr>' + '<tr><td>Email Address</td><td>' + rEmailAddress + '</td></tr>' + '<tr><td>Address</td><td>' + rAddress + '</td></tr>').appendTo(playerInfo);

    var testNumber = 0;
    // for showing player's history
    for (i = 0; i < quizMaster["results"].length; i++) {
        if (quizMaster["results"][i]['myEmailAddress'] == rEmailAddress) {
            ++testNumber;
